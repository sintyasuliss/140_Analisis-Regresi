...
x1 <- runif(n,5,15)
x2 <- rnorm(n,3,3) #rata2 3 dan ragam 3
x0 <- rep(1,n)
...
b <- solve(t(X)%*%X)%*%t(X)%*%y;round(b,4) #pendugaan terhadap Beta
```
...
```{r}
sigma_kuadrat <- (t(y)%*%y-t(b)%*%t(X)%*%y)/(n-p) #y'y-t'x'y
Res_se <- sqrt(sigma_kuadrat) #standar error residual
round(Res_se,3)
```
```{r}
df <- n-p #db galat
...
galat <- y-(b0+b1*x1+b2*x2) #y aktual - y duga
KTG   <- sum(galat^2)/(n-p)
...
```{r}
pf(Fhit, dbreg, dbg, lower.tail = F) #p value
```
...
se_b <- sqrt(diag(cov_beta)); round(se_b,4) #mencari standar error
```
...
rownames(Selang.Kepercayaan ) <- c("Intersept atau b0", "b1", "b2")
Selang.Kepercayaan
```
